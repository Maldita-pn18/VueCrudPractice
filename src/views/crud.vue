  ...
  <div>
    ...
    <div :style="updateForm">
        ...
        <select v-model="updateFormsVariables.gender">
            <option value="male" :selected="(updateFormsVariables.gender === 'male') ? 'selected' : ''">Male</option>
            <option value="female" :selected="(updateFormsVariables.gender === 'female') ? 'selected' : ''">Female</option>
        </select>
        <br>
...
//   },
    ...
    mounted(){
        ...
        this.$axios.post('http://localhost:8000/retrieve', {token: localStorage.getItem('token')}).then(response => {
    ...
    methods:{
        ...
        update(){
            ...
            this.$axios.post('http://localhost:8000/update', updateInfo).then(response=>{
        ...
        //       }
        //   })
            this.$axios.post('http://localhost:8000/delete/'+data.id).then(response=>{
        ...
        createData(){
            if(this.createFormsVariables.name !== '' && this.createFormsVariables.age !== '' && this.createFormsVariables.gender !== ''){
                let informations = this.createFormsVariables
                informations['token'] = localStorage.getItem('token')
                this.$axios.post('http://localhost:8000/create', informations).then(response=>{
